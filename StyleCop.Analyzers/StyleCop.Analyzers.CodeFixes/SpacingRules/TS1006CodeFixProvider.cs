...
namespace StyleCop.Analyzers.SpacingRules
{
   ...
   /// </remarks>
   [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(TS1006CodeFixProvider))]
    [Shared]
    internal class TS1006CodeFixProvider : CodeFixProvider
    {
        /// <inheritdoc/>
        public override ImmutableArray<string> FixableDiagnosticIds { get; } =
            ImmutableArray.Create(TS1006SubscribingShouldBeAligned.DiagnosticId);
        ...
        private static async Task<Document> FixWhitespaceAsync(Document document, Diagnostic diagnostic, double dAlignmentStandardDeviation, CancellationToken cancellationToken)
        {
            ...
            int nCharToAdd = await AlignmentHelper.AdditionalSpacesToAddForConditionAsync( root, diagnostic, dAlignmentStandardDeviation, AlignmentHelper.IsSimpleSubscribing, SyntaxKind.PlusEqualsToken, cancellationToken ).ConfigureAwait( false );
        ...
        private class FixAll : DocumentBasedFixAllProvider
        {
            ...
            protected override async Task<SyntaxNode> FixAllInDocumentAsync(FixAllContext fixAllContext, Document document, ImmutableArray<Diagnostic> diagnostics)
            {
                ...
                for (int nDiagnostic = diagnostics.Length; nDiagnostic-->0; )
                {
                   ...
                   int nCharToAdd = await AlignmentHelper.AdditionalSpacesToAddForConditionAsync( root, diagnostic, settings.AlignmentRules.AlignmentStandardDeviation, AlignmentHelper.IsSimpleSubscribing, SyntaxKind.PlusEqualsToken, fixAllContext.CancellationToken ).ConfigureAwait( false );
