...
namespace StyleCop.Analyzers.SpacingRules
{
   ...
   [DiagnosticAnalyzer(LanguageNames.CSharp)]
    internal class TS1005AssignmentsShouldBeAligned : DiagnosticAnalyzer
    {
        /// <summary>
        /// The ID for diagnostics produced by the <see cref="SA1025CodeMustNotContainMultipleWhitespaceInARow"/>
        /// analyzer.
        /// </summary>
        public const string DiagnosticId = "TS1005";
        private static readonly LocalizableString Title = "Assignments must be aligned :)";
        private static readonly LocalizableString Description = "Assignments should be lined up for better readability :)";
        private static readonly string HelpLink = "https://github.com/TechSmith/CamtasiaWin/wiki/Automated-Code-Standards";

        private const string MessageNotFollowed = "Assignments should be aligned :)";
        ...
        private static void HandleSyntaxTree(SyntaxTreeAnalysisContext context, StyleCopSettings settings )
        {
            ...
            foreach (var token in root.DescendantTokens(descendIntoTrivia: true).Where(t => t.IsKind(SyntaxKind.EqualsToken)))
            {
               if ( AlignmentHelper.IsSimpleAssignment( root, token) )
               {
                  ...
                  int nAdditonalSpaces = AlignmentHelper.AdditionalSpacesToAddForConditionAsync( root, nMyLine, nMyColumn, settings.AlignmentRules.AlignmentStandardDeviation, AlignmentHelper.IsSimpleAssignment, SyntaxKind.EqualsToken ).Result;
