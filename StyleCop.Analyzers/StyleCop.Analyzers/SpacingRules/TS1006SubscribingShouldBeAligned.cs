...
namespace StyleCop.Analyzers.SpacingRules
{
   ...
   [DiagnosticAnalyzer(LanguageNames.CSharp)]
    internal class TS1006SubscribingShouldBeAligned : DiagnosticAnalyzer
    {
        /// <summary>
        /// The ID for diagnostics produced by the <see cref="SA1025CodeMustNotContainMultipleWhitespaceInARow"/>
        /// analyzer.
        /// </summary>
        public const string DiagnosticId = "TS1006";
        private static readonly LocalizableString Title = "Subscribing must be aligned :)";
        private static readonly LocalizableString Description = "Subscribing should be lined up for better readability :)";
        private static readonly string HelpLink = "https://github.com/TechSmith/CamtasiaWin/wiki/Automated-Code-Standards";

        private const string MessageNotFollowed = "Subscribing should be aligned :)";
        ...
        private static void HandleSyntaxTree(SyntaxTreeAnalysisContext context, StyleCopSettings settings )
        {
            ...
            foreach (var token in root.DescendantTokens(descendIntoTrivia: true).Where(t => t.IsKind(SyntaxKind.PlusEqualsToken)))
            {
               if ( AlignmentHelper.IsSimpleSubscribing( root, token) )
               {
                  ...
                  int nAdditonalSpaces = AlignmentHelper.AdditionalSpacesToAddForConditionAsync( root, nMyLine, nMyColumn, settings.AlignmentRules.AlignmentStandardDeviation, AlignmentHelper.IsSimpleSubscribing, SyntaxKind.PlusEqualsToken ).Result;
